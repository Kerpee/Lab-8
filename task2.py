...
import cv2
import numpy as np
THRESHOLD = 0.6  # Вводим константу для обозначения соответствия объектов с меткой
...
while True:
    ...
    if not ret:  # Проверяется корректность считывание кадра
        break
    gray_frame = cv2.cvtColor(frame, cv2.COLOR_RGB2GRAY)  # Перевод кадра в серый цвет для более простого распознавания
